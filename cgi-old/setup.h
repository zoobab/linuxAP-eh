...
// Default config keyword separator
#define SEP '='
...
#define out_config(num, f, kw, sep) \
	do { char *val=getenv(CGI_keyword(kw,num)); \
	     fprintf(f,"%s%s\n",CFG_keyword(kw,num,sep),!val ? "" : val); } while(0)

#define CGI_keyword(keywords, num) \
	({ \
	   sprintf(bigbuf,"CGI_%s",keywords[num]); \
	   bigbuf; \
	})
#define CFG_keyword(keywords, num, sep) \
	({ \
	   sprintf(bigbuf,"%s%c",keywords[num],sep); \
	   bigbuf; \
	})

char *conf_keyword (int num, char **keywords[], char sep);
char **get_file_chooser (char *dirname, char *value, char *keyword, char *newmsg);
int read_cfg (char *filename, char *keywords[], char sep, char *values[]);
void read_cfg_value (char *keywords[], char sep, char *values[], char *line);
char *retrieve_cfg_value(char **keywords, int ix, char sep, char *line);
int readline(FILE *f, char *buffer, int size);
int argvindex(char **haystack, char *needle, int (*strfunc)());
int argvindex_sep(char **haystack, char sep, char *needle, int (*strfunc)());
