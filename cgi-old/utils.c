...
int read_cfg (char *filename, char *keywords[], char sep, char *values[]) {
	...
	if (line[0]!='#' && line[0]!='\n' && line[0]!='\0') // skip non-values
	    read_cfg_value(keywords, sep, values, line);
    }
    fclose(f);
    return 1;
}
...
void read_cfg_value (char *keywords[], char sep, char *values[], char *line) {
    int ix = argvindex_sep(keywords,sep,line,strncmp);
    values[ix] = retrieve_cfg_value(keywords,ix,sep,bigbuf);
}
...
char *retrieve_cfg_value(char **keywords, int ix, char sep, char *line) {
	char *p,*q;
	p = line + strlen(CFG_keyword(keywords,ix,sep));
	while(isspace(*p)) p++;
...
int argvindex_sep(argvlist, sep, s, strfunc)
char *argvlist[];
char sep;
char *s;
int (*strfunc)();
{
    ...
    char *kwd;
    for(index = 0; argvlist[index] != NULL; index++ ) {
	kwd = CFG_keyword(argvlist,index,sep);
        if ( (strfunc)(kwd, s, strlen(kwd)) == 0)
            break;
    }
    return(index);
}
