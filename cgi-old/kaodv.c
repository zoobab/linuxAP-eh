...
static void kaodv_write()
{
    ...
    fprintf(f,"\n\
# Use Kernel AODV\n");
    out_config(KV_KAODV,f,kkw,SEP);

    fprintf(f,"\n\
# Use internet gatewaying mode\n");
    out_config(KV_KAODV_GW,f,kkw,SEP);

    fprintf(f,"\n\
# Use loopback\n");
    out_config(KV_KAODV_LO,f,kkw,SEP);
...
# if no interfaces present, will use all\n");
    out_config(KV_KAODV_IF,f,kkw,SEP);

    fprintf(f,"\n\
# Block interfaces' Hello Messages\n");
    out_config(KV_KAODV_BL,f,kkw,SEP);
...
# (will disable hello sending)\n");
    out_config(KV_KAODV_GW_GIF,f,kkw,SEP);

    fprintf(f,"\n\
# AODV interface\n");
    out_config(KV_KAODV_GW_AIF,f,kkw,SEP);

    fprintf(f,"\n\
# AODV subnet (0's as wildcards)\n");
    out_config(KV_KAODV_GW_SUBNET,f,kkw,SEP);
...
{
    kaodv_gw_sel = getenv(CGI_keyword(kkw,KV_KAODV_GW));
    kaodv_menu();
    printf("</html>\n");
    exit(0);
}
